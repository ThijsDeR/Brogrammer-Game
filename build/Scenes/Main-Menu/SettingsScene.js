...
export default class SettingsScene extends Scene {
                ...
                if (prop instanceof Slider) {
                    if (prop.isHovered({ x: event.x, y: event.y })) {
                        ...
                        if (prop.getId() === 'masterSound')
                            this.userData.changeSoundProcent(UserData.MASTER_SOUND_OBJECT_NAME, prop.getPosition());
                        else if (prop.getId() === 'musicSound')
                            this.userData.changeSoundProcent(UserData.MUSIC_SOUND_OBJECT_NAME, prop.getPosition());
                        else if (prop.getId() === 'uiSound')
                            this.userData.changeSoundProcent(UserData.UI_SOUND_OBJECT_NAME, prop.getPosition());
    ...
    draw() {
        ...
        Scene.writeTextToCanvas(this.ctx, 'Instellingen', this.canvas.width / 2, this.canvas.height / 10, this.canvas.height / 20, 'white');
    }
    processInput() {
    }
