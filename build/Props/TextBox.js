...
export default class TextBox {
    ...
    width;
    height;
    constructor(xPos, yPos, width, height, sentences, textBoxImage) {
        this.xPos = xPos;
        this.yPos = yPos;
        this.width = width;
        this.height = height;
    ...
    draw(ctx) {
        ...
        if (!this.isDone()) {
            Scene.writeTextToCanvas(ctx, this.sentences[this.currentSentence].substring(0, this.sentenceLength), this.xPos + (this.width / 2), this.yPos + (this.height / 2), this.height / 10, 'black', 'center', 'middle');
        }
    }
