...
export default class DoodleScene extends GameLevel {
  ...
  public constructor(canvas: HTMLCanvasElement, userData: UserData) {
    ...
    this.props = [
      ...
      new FallLine(
        0,
        this.canvas.height - (this.canvas.height / 100),
        this.canvas.width,
      ...
      new CloudPlatform(
        ...
        this.canvas.height - this.canvas.height / 20,
        canvas.width - (this.canvas.width / 10) * 2,
      ...
      new CloudPlatform(
        this.canvas.width / 10,
        DoodleInfo.LEVEL_YPOS_FINISH * this.canvas.height,
        canvas.width - (this.canvas.width / 10) * 2,
        this.canvas.height / 10,
      ),
    ];

    this.sonNPC = new SonNPC(
      (this.canvas.width / 2) - (this.canvas.width / 40),
      (DoodleInfo.LEVEL_YPOS_FINISH * this.canvas.height) - (this.canvas.height / 10),
  ...
  public createProps(): void {
    ...
    while (i < 1000 && atFinish === false) {
      const xPos = Game.randomNumber(
        this.canvas.width / 8,
        this.canvas.width - this.canvas.width / 8,
      );

      const yPos = Game.randomNumber(
        previousHeight + (this.canvas.height / 5),
        previousHeight + ((this.canvas.height / 10) * 3),
      );
      ...
      const questionYPos = Game.randomNumber(
        previousQuestionHeight + (this.canvas.height * 5),
        previousQuestionHeight + (this.canvas.height * 8),
      );
